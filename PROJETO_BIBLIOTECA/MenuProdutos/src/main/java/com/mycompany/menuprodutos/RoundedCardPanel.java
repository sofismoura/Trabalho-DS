...
public class RoundedCardPanel extends JPanel {
    ...
    private Color backgroundColor = new Color(250, 250, 250); // branco
    private Color borderColor = new Color(173, 216, 230); 
    ...
    public RoundedCardPanel(Produto produto, Menu_Produtos menuProdutos) {
        ...
        comprarButton.addActionListener(e -> {
            Menu menu = new Menu(produto, menuProdutosReferencia); 
...
private ImageIcon carregarImagemDoBanco(Produto produto) {
    try {
        byte[] imagemBytes = BancoDeDados.carregarImagemProduto(produto.getId());
        if (imagemBytes != null) {
    ...
    if (produto.getCaminhoImagem() != null && !produto.getCaminhoImagem().isEmpty()) {
        try {
            ...
            System.err.println("Erro ao carregar imagem do caminho: " + e.getMessage());
        }
    }
    
    return new ImageIcon("src/assets/images/default-product.png");
}
